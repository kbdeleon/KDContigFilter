package KDContigFilter::KDContigFilterClient;
...
KDContigFilter::KDContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => KDContigFilter::KDContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a KDContigFilter.FilterContigsParams
$output is a KDContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a KDContigFilter.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a KDContigFilter.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "KDContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for KDContigFilter::KDContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "KDContigFilter::KDContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package KDContigFilter::KDContigFilterClient::RpcClient;
